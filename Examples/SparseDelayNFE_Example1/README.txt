To make parallel version type "make SparseDelayNFE_Example1_PAR".  **ONLY WORKS IF OPENMP IS INSTALLED**


SparseDelayNeuralField problem assumes delays of the form:
...
For each delay tau_ij, only the ith and jth row of the tau_ij column of the Z matrix are required. Hence, a sparse matrix implementation of Z can be used in order to save on computations and memory. The locations of the entries in the Z matrix must be provided in column-major ordering.
