...
void DelayRungeKuttaSolver::DelayRungeKuttaAlgorithm(const arma::mat& a, const arma::vec& b, const arma::vec& c)
{
    // Set current time
    double time = mInitialTime;

    // Set up delays
    arma::mat delayStates(mNumDelayEqs, mDelays.n_elem);
    double delayTime;
    arma::vec historyTemp(mNumDelayEqs);
    for (arma::uword i = 0; i < mDelays.n_elem; ++i)
    {
        delayTime = time - mDelays(i);
        mpDDESystem->ComputeHistory(delayTime, historyTemp);
        delayStates.col(i) = historyTemp;
    }
        ...
        for (arma::uword i = 0; i < a.n_rows; ++i)
        {
            ...
            for (arma::uword j = 0; j < mDelays.n_elem; ++j)
            {
                ...
                if (delayTime < mInitialTime)
                {
                    ...
                    mpDDESystem->ComputeHistory(delayTime, histTemp);

                    delayStates.col(j) = histTemp;
                }
                else
                {
                    delayStates.col(j) = historyBuffer.ComputeDelayState(delayTime);
                }
            }

            mpDDESystem->ComputeF(time + mStepSize * c(i), *mpState + mStepSize * temp_sum, delayStates, temp_F);
...
void DelayRungeKuttaSolver::AdaptiveDelayRungeKuttaAlgorithm(const arma::mat& a, const arma::vec& b, const arma::vec& c, const arma::vec& bHat, const double ATol, const double RTol)
{
    // Set current time
    double time = mInitialTime;
    ...
    double delayTime;
    arma::vec historyTemp(mNumDelayEqs);
    for (int i = 0; i < mDelays.n_elem; ++i)
    {
        delayTime = time - mDelays(i);
        mpDDESystem->ComputeHistory(delayTime, historyTemp);
        delayStates.col(i) = historyTemp;
...
void DelayRungeKuttaSolver::SetStepSizeMin(const double stepSizeMin)
{
    mStepSizeMin = stepSizeMin;
}

void DelayRungeKuttaSolver::SetFacMin(const double facMin)
{
    mFacMin = facMin;
}

void DelayRungeKuttaSolver::SetFac(const double fac)
{
    mFac = fac;
}

void DelayRungeKuttaSolver::SetFacMax(const double facMax)
{
    mFacMax = facMax;
}
