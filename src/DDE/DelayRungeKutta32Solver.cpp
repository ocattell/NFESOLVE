    ...
    const arma::uvec& outputIndices)
{
    DebugCheck(numDelayEqs > initialState.n_elem, "DelayRungeKutta32Solver(): numDelayEquations is greater than the system size");
    DebugCheck(delays.min() < 1e-9, "DelayRungeKutta32Solver(): Minimum delay value too small");
    DebugCheck(outputIndices.max() >= initialState.n_elem, "DelayRungeKutta32Solver(): outputIndices value out of bounds. It should only contain values between 0 and " + std::to_string(initialState.n_elem - 1));
    DebugCheck(outputIndices.is_empty(), "DelayRungeKutta32Solver(): outputIndices must be non-empty");
    DebugCheck(arma::any(outputIndices != arma::unique(outputIndices)), "DelayRungeKutta32Solver(): outputIndices must only contain unique elements");
